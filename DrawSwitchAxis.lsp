...
;; == ВИПРАВЛЕНО: МАРКЕР ЦСП ТА ОСІ ТЕПЕР СТВОРЮЮТЬСЯ НА Z=0.0 == (НОВЕ!)
;; ============================================================
...
                                 temp_pt1 temp_pt2 ) ; Додані тимчасові змінні для маркера ЦСП
...
      
      ;; Створюємо нову пряму полілінію P1 - P2_proj - P4 в 2D (Всі точки на Z=0)
      (setq p2_proj_for_pline (list (car proj_pt_p2)
                                    (cadr proj_pt_p2)
                                    0.0)) ; Забезпечуємо Z=0 для полілінії
      
...
  (setq p5_projected_on_branch (list (car p5_projected_on_branch) (cadr p5_projected_on_branch) 0.0)) ; Забезпечуємо Z=0
...
      ;; Створюємо нову, коректну полілінію від ЦСП до спроектованої P5 (в 2D, на Z=0)
...
  (setq csp_marker_length 0.76) ; Довжина маркера
...
  ;; Розраховуємо кінці маркера ЦСП, центруючи його на csp_pt (на Z=0)
  (setq temp_pt1 (polar csp_pt perp_angle (/ csp_marker_length 2.0)))
  (setq temp_pt2 (polar csp_pt (+ perp_angle pi) (/ csp_marker_length 2.0))) 
  
  ;; *** ВИПРАВЛЕННЯ: Явно встановлюємо Z-координату для точок маркера на 0.0 ***
...
          (vla-put-color csp_marker_obj 256) ; Встановлюємо колір на ByLayer (або 1 для червоного для тесту)
...
  (setq contour_pt3 (list (car contour_pt3) (cadr contour_pt3) 0.0)) ; Забезпечуємо Z=0
...
          (command "_.INSERT"
                   block_name
                   final_p2_target_pt ; <--- Використовуємо кінцеву, зміщену точку P2 (з оригінальною Z)
                   1.0 ; Scale X
                   1.0 ; Scale Y
