...
;; |                                                                                               |
;; | Версія: 2.2 (Виправлено помилку "numberp: nil")                                               |
;; | Дата: 10.06.2025                                                                              |
;; |                                                                                               |
;; | Опис:                                                                                         |
;; | Скрипт вимірює відстань між двома точками і створює текст з результатом.                       |
;; | Текст автоматично повертається паралельно до виміряної лінії.                                 |
...
(defun c:DistText ( / *error* old_cmdecho p1 p2 dist dist_str text_ins_pt text_style text_height text_color text_angle pi_val)
  ...
  (setq text_color 5)
  ;; === ЗМІНЕНО: Жорстко задаємо значення PI для надійності ===
  ...
  (while 
    ...
    (if (setq p2 (getpoint p1 "\nВкажіть другу точку: "))
      (progn ; Виконується, якщо вказано обидві точки
        ...
        ;; Перевірка, чи текст не буде перевернутим (кут між 90 та 270 градусів)
...
(princ "\nКоманду 'DistText' v2.2 (DT) з виправленням помилки завантажено.")
(princ)
