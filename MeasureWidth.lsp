...
;; |                                                                                               |
;; | Версія: 2.3 (Додано корекцію кута на -90° для вертикальних шрифтів)                           |
;; | Дата: 10.06.2025                                                                              |
;; |                                                                                               |
;; | Опис:                                                                                         |
;; | Скрипт вимірює відстань і створює текст, повернений паралельно до лінії.                       |
;; | Враховує можливий поворот на 90° через налаштування стилю тексту (@-шрифт).                   |
;; | Використовує поточні налаштування прив'язок (OSMODE).                                         |
;; |                                                                                               |
  ...
  (while 
    ...
    (if (setq p2 (getpoint p1 "\nВкажіть другу точку: "))
      (progn ; Виконується, якщо вказано обидві точки
        ...
        ;; Обчислення кута для повороту тексту
        (setq text_angle (angle p1 p2))
        
        ;; === НОВА КОРЕКЦІЯ: Віднімаємо 90 градусів (pi/2) для компенсації @-шрифта ===
        (setq text_angle (- text_angle (/ pi_val 2.0)))

        ;; Перевірка, чи текст не буде перевернутим (з урахуванням нової корекції)
        (if (and (> (angle p1 p2) (/ pi_val 2.0)) (< (angle p1 p2) (* 1.5 pi_val)))
          (setq text_angle (+ text_angle pi_val)) ; Якщо так, повертаємо на 180 градусів
        )
...
(princ "\nКоманду 'DistText' v2.3 (DT) з корекцією кута завантажено.")
(princ)
