...
;; |                                                                                               |
;; | Версія: 2.4 (Виправлено логіку повороту тексту)                                                |
;; | Дата: 10.06.2025                                                                              |
;; |                                                                                               |
;; | Опис:                                                                                         |
;; | Скрипт вимірює відстань і створює текст, повернений паралельно до лінії.                       |
;; | Коректно обробляє читабельність (щоб не був догори дриґом) та компенсує                       |
;; | можливий поворот на 90° через налаштування стилю тексту (@-шрифт).                            |
;; |                                                                                               |
  ...
  (while 
    ...
    (if (setq p2 (getpoint p1 "\nВкажіть другу точку: "))
      (progn ; Виконується, якщо вказано обидві точки
        ...
        ;; === ОНОВЛЕНИЙ БЛОК ОБЧИСЛЕННЯ КУТА ===
        ...
        ;; Крок 3: І тільки тепер застосовуємо корекцію на -90° для @-шрифтів
...
(princ "\nКоманду 'DistText' v2.4 (фінальна версія) завантажено.")
(princ)
