...
;; |                                                                                               |
;; | Версія: 4.0 (Текст розташовується перпендикулярно)                                            |
;; | Дата: 10.06.2025                                                                              |
;; |                                                                                               |
;; | Опис:                                                                                         |
;; | Скрипт вимірює відстань і створює текст, повернений **перпендикулярно** до виміряної лінії.    |
;; | Коректно обробляє читабельність (щоб не був догори дриґом).                                    |
;; | Призначений для роботи з коректно налаштованими стилями тексту.                               |
;; |                                                                                               |
  ...
  (while 
    ...
    (if (setq p2 (getpoint p1 "\nВкажійте другу точку: "))
      (progn ; Виконується, якщо вказано обидві точки
        ...
        ;; === БЛОК ОБЧИСЛЕННЯ ПЕРПЕНДИКУЛЯРНОГО КУТА ===
        ;; Крок 1: Обчислюємо базовий, паралельний кут
        (setq text_angle (angle p1 p2))
        
        ;; Крок 2: Забезпечуємо читабельність паралельного кута (поворот на 180°)
        (if (and (> text_angle (/ pi_val 2.0)) (< text_angle (* 1.5 pi_val)))
          (setq text_angle (+ text_angle pi_val))
        )
        
        ;; Крок 3: Повертаємо на 90° для отримання перпендикуляра
...
(princ "\nКоманду 'DistText' v4.0 (перпендикулярний текст) завантажено.")
(princ)
