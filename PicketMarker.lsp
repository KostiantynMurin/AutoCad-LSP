...
(vl-load-com) ; Завантажуємо підтримку ActiveX/COM
...
                             xdata_app_name dictionaries app_id_obj)

  (princ "\n*** Running CREATE_PICKET_MARKER v2025-04-25_UseBlock_RotateFixY_RemAngle ***")

  ;; Налаштування констант
  (setq target_layer   "0"
        fuzz           1e-9
        xdata_app_name "PicketMaster"
  )

  ;; Перевизначення обробника помилок
  (defun *error* (msg)
    (if old_vars (mapcar 'setvar (mapcar 'car old_vars) (mapcar 'cdr old_vars)))
    (if (not (member msg '("Function cancelled" "quit / exit abort" "Відміна користувачем" "Block check failed" "AutoCAD Application недоступний" "ActiveDocument недоступний")))
...
  ;; Перевірка та ініціалізація об'єкта AutoCAD Application
  (if (null vl-acad-app)
    (vl-load-com) ; Перезавантажуємо COM, якщо не завантажено
  )
  (setq acad_obj (vlax-get-acad-object T)) ; Примусово отримуємо об'єкт AutoCAD Application
  (if (or (null acad_obj) (vlax-object-released-p acad_obj))
    (progn
      (princ "\n*** ПОМИЛКА: Не вдалося отримати об'єкт AutoCAD Application. Можливо, AutoCAD не повністю ініціалізовано.")
      (*error* "AutoCAD Application недоступний")
    )
  )
  (setq doc (vla-get-ActiveDocument acad_obj))
  (if (or (null doc) (vlax-object-released-p doc))
    (progn
      (princ "\n*** ПОМИЛКА: Не вдалося отримати об'єкт ActiveDocument. Можливо, креслення не відкрито або неактивне.")
      (*error* "ActiveDocument недоступний")
    )
  )

  ;; Реєстрація App Name для XDATA
...
  (if (vl-catch-all-error-p (vlax-invoke dictionaries 'Item xdata_app_name))
    (progn
